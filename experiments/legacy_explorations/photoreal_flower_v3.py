...
def render_photoreal(filename):
    ...
    # --- ADD THE BUD (STAMEN) ---
    bud_radius = 0.25
    bud_mask = (r < bud_radius)
    
    # Bud is a raised dome that sits on top of all the petals
    max_petal_z = num_petals * 0.15
    bud_h = max_petal_z + 0.2 - 6.0 * r**2
    
    # High frequency seed texture (Fibonacci spiral approximation)
    seeds = torch.cos(150 * X + 40 * Y) * torch.cos(150 * Y - 40 * X)
    # The seeds add physical bumps to the height map!
    bud_h += 0.06 * seeds * torch.clamp(1.0 - (r / bud_radius), 0, 1)
    
    update_mask = (bud_h > H) & bud_mask
    H = torch.where(update_mask, bud_h, H)
    LayerID = torch.where(update_mask, torch.full_like(r, 99.0), LayerID) # Unique layer ID for the bud
    
    bud_color = torch.tensor([0.15, 0.05, 0.0]).view(3, 1, 1)
    gold_highlight = torch.tensor([0.7, 0.5, 0.0]).view(3, 1, 1)
    b_color = bud_color * (1.0 - torch.clamp(seeds, 0, 1)) + gold_highlight * torch.clamp(seeds, 0, 1)
    
    for c in range(3):
        color_map[c] = torch.where(update_mask, b_color[c] * torch.ones_like(r), color_map[c])
    # ----------------------------
...
render_photoreal("scratch/photoreal_flower_v3.png")
print("Rendered Photorealistic Flower V3 with Bud.")
